...
public class BasicFieldUDF extends UDF {
    public String evaluate(String line, String jsonKeyString){

        StringBuilder sb = new StringBuilder();

        String[] jsonKeys = jsonKeyString.split(",");
        String[] logContext = line.split("\\|");

        if (logContext.length != 2 || StringUtils.isBlank(logContext[1])) {
            return "";
        }

        try {
            JSONObject jsonObject = new JSONObject(logContext[1]);
    ...
    public static void main(String[] args) {
        ...
        String evaluate = new BasicFieldUDF().evaluate(line, "mid,uid,vc,vn,l,sr,os,ar,md,ba,sv,g,hw,nw,ln,la,t");
